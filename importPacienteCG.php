...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        $camaQuery1 = "SELECT numero FROM camas WHERE numero IN ('$camaStr1') AND area = 'CIRUGÍA GENERAL'";
        ...
        if (!empty($noExisten)) {
            echo "Las siguientes camas no existen en el área de CIRUGÍA GENERAL, valide los datos: " . implode(", ", $noExisten);
            ...
            $camaQuery = "SELECT cama FROM pacientes WHERE cama IN ('$camaStr') AND area = 'CIRUGÍA GENERAL' AND statusP = 'Activo'";
            ...
            if (mysqli_num_rows($checkResultCama) > 0) {
                ...
                foreach ($data as $row) {
                    ...
                    VALUES ('', '$nombre', '$fechaNacimiento', '$idPaciente', '$cama', '$edadAnios', '$edadMeses', '$edadDias', '$diagnosticoMed', '$prescripcionNutri', 'CIRUGÍA GENERAL', '$vip', '$observaciones', '$controlTamizaje', '$sesionNombre', 'Activo')";
