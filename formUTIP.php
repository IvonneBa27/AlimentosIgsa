...
function guardarOActualizarDieta($idPaciente, $datos, $rangoHoraActual)
{
    ...
    if ($query->num_rows > 0) {
        ...
        VALUES ('', '$nombre', '$fechaNacimiento','$idPaciente', '$cama', '$edad', '$diagnosticoMed', '$prescripcionNutri', '$desayuno', '$colMatutina', '$comida', '$colVespertina', '$cena', '$colNocturna', '$observaciones', '$controlTamizaje', 'UTIP', '$vip', '$fechaHoraActual', '$usuario')";
...
WHERE p.statusP = 'Alta' AND p.area = 'UTIP'ORDER BY 
CASE 
                            ...
                            <form class="row g-3 needs-validation" method="POST" action="" id="myFormAddPaciente">
                                ...
                                <div class="form-group col-md-9">
                                    ...
                                    <select name="cama" id="cama" class="form-control fs-5" required>
                                        <?php
                                        $sqlCama = "SELECT numero FROM camas WHERE area = 'UTIP'";
