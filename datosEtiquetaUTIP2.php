...
if (in_array('Todos', $pacientes)) {
    $query = "SELECT * FROM dietas WHERE area = 'UTIP' AND DATE(Fecha_Hora_Creacion) = CURDATE()";
    $result = mysqli_query($con, $query);
    $datos = mysqli_fetch_all($result, MYSQLI_ASSOC);
} else {
    foreach ($pacientes as $paciente) {
        $query = "SELECT * FROM dietas WHERE area = 'UTIP' AND Nombre_Paciente = '$paciente' AND DATE(Fecha_Hora_Creacion) = CURDATE()";
        $result = mysqli_query($con, $query);
