    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        foreach ($data as $row) {
            ...
            if (count($row) >= 11) {
                ...
                $diagnosticoMed = mysqli_real_escape_string($con, $row[5]);
                $prescripcionNutri = mysqli_real_escape_string($con, $row[6]);
                $area = mysqli_real_escape_string($con, $row[7]);
                $vip = mysqli_real_escape_string($con, $row[8]);
                $observaciones = mysqli_real_escape_string($con, $row[9]);
                $controlTamizaje = mysqli_real_escape_string($con, $row[10]);

                // Convertir la fecha al formato YYYY-MM-DD
                $fechaNacimiento = date('Y-m-d', strtotime($fechaNacimiento));

                $sql = "INSERT INTO pacientes (id, nombre, fechaNacimiento, idPaciente, cama, edad, diagnosticoMed, prescripcionNutri, area, vip, observaciones, controlTamizaje, creadoPor, statusP) 
                    VALUES ('', '$nombre', '$fechaNacimiento', '$idPaciente', '$cama', '$edad', '$diagnosticoMed', '$prescripcionNutri', 'GINECOLOGÍA', '$vip', '$observaciones', '$controlTamizaje', '$sesionNombre', 'Alta')";
        ...
        foreach ($data as $row) {
            ...
            if (count($row) >= 10) {
                ...
                $sql = "INSERT INTO pacientes (id, nombre, fechaNacimiento, idPaciente, cama, edad, edadMeses, edadDias, diagnosticoMed, prescripcionNutri, area, vip, observaciones, controlTamizaje, creadoPor, statusP) 
                    VALUES ('', '$nombre', '$fechaNacimiento', '$idPaciente', '$cama', '$edadAnios', '$edadMeses', '$edadDias', '$diagnosticoMed', '$prescripcionNutri', 'GINECOLOGÍA', '$vip', '$observaciones', '$controlTamizaje', '$sesionNombre', 'Alta')";
