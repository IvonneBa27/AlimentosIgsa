...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = $_POST['id'];
    $tipoUsuario = strtoupper(trim($_POST['tipo_usuario']));

    // Validar que los datos no estén vacíos
    if (!empty($id) && !empty($tipoUsuario)) {
        try {
            // Crear la consulta SQL
            $sql = "UPDATE tipo_usuario SET tipo_usuario = ? WHERE id = ?";
            $stmt = $con->prepare($sql);

            // Verificar si la consulta fue preparada correctamente
            if ($stmt) {
                // Vincular los parámetros
                $stmt->bind_param("si", $tipoUsuario, $id); // "s" para string, "i" para integer
