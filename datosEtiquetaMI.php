...
if ($tipo == 'Desayuno' && $paciente == 'Todos') {
    ...
    $query = "SELECT * FROM dietas WHERE area = 'MEDICINA INTERNA' AND DATE(Fecha_Hora_Creacion) = CURDATE()";
    ...
    $query = "SELECT * FROM dietas WHERE area = 'MEDICINA INTERNA' AND Nombre_Paciente = '$paciente' AND DATE(Fecha_Hora_Creacion) = CURDATE()";
    $result = mysqli_query($con, $query);
    $datos = mysqli_fetch_all($result, MYSQLI_ASSOC);
    $generarEtiqueta = 'generarEtiquetaDesayuno';
