...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Crear la consulta SQL con marcador de parámetro de MySQLi (?)
        $sql = "UPDATE producto SET estatus = 3 WHERE id = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: adminProducto.php?msg=updated");
                exit();
            } else {
                // Manejo de error si la consulta no se ejecutó correctamente
                echo 'Error al actualizar el estado del producto: ' . $stmt->error;
            }
