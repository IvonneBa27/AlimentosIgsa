...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE salas_conferencias SET estatus_id = 2 WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: adminSalas.php?msg=updated");
            exit();
        } else {
            // Manejo de error si la consulta no se ejecutó correctamente
            echo 'Error al actualizar el estado de la sala de conferencia: ' . $stmt->error;
        }
