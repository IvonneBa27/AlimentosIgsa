...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['file'])) {
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        $camaQuery1 = "SELECT numero FROM camas WHERE numero IN ('$camaStr1') AND area = 'QUEMADOS'";
        ...
        if (!empty($noExisten)) {
            echo "Las siguientes camas no existen en el área de QUEMADOS, valide los datos: " . implode(", ", $noExisten);
            ...
            $camaQuery = "SELECT cama FROM pacientes WHERE cama IN ('$camaStr') AND area = 'QUEMADOS' AND statusP = 'Activo'";
            ...
            if (mysqli_num_rows($checkResultCama) > 0) {
                ...
                echo "La cama ingresada ya existe en el área de QUEMADOS, valide los datos: " . implode(", ", $existingCamas);
            } else {
                ...
                foreach ($data as $row) {
                    ...
                    VALUES ('', '$nombre', '$fechaNacimiento', '$idPaciente', '$cama', '$edadAnios', '$edadMeses', '$edadDias', '$diagnosticoMed', '$prescripcionNutri', 'QUEMADOS', '$vip', '$observaciones', '$controlTamizaje', '$sesionNombre', 'Activo')";
