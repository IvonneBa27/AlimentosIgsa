...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE tipo_usuario SET status_id = 2 WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: adminUsuario.php?msg=updated");
            exit();
        } else {
            // Manejo de error si la consulta no se ejecutó correctamente
            echo 'Error al actualizar el estado de la empresa: ' . $stmt->error;
        }
