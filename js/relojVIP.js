...
function guardarCambio(idPaciente, datos) {
    ...
    if (confirmar) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'formVIP.php', true);
        ...
        theadRows.forEach((tr, index) => {
            ...
            if (index === 0) {
                th.innerHTML = `<span>Solicitud día siguiente</span><br><button class="btn btn-success btn-sm mt-1" onclick="enviarDatosBloqueo()">Enviar datos</button>
                                <br><button class="btn btn-primary btn-sm mt-1" onclick="window.open('pdfDesayunoVIP.php', '_blank')">Exportar</button>`;
            } else {
...
function cargarDatosBloqueo() {
    fetch("guardar_bloqueosVIP.php") // o "obtener_bloqueosCG.php" si lo tienes separado
        .then(response => response.json())
        .then(data => {
            const filas = document.querySelectorAll("#miTabla tbody tr");
        ...
        fetch('importPacienteVIP.php', {
            ...
            .then(text => {
                if (text.includes('Datos insertados correctamente.')) {
                    ...
                    //window.location.replace("http://localhost/dietas/formVIP.php");
                    //window.location.replace("http://10.1.7.240:8080/dietas/formVIP.php");
                    //window.location.replace("http://10.1.7.169/dietas/formVIP.php");

                } else if (text.includes('El ID ingresado para paciente ya existe')) {
                    alert(text);
                } else if (text.includes('Las siguientes camas no existen en el área de PRIVADOS')) {
                    alert(text);
                } else if (text.includes('La cama ingresada ya existe en el área de PRIVADOS')) {
                    alert(text);
                } else {
