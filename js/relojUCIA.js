...
function guardarCambio(idPaciente, datos) {
    ...
    if (confirmar) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'formUCIA.php', true);
...
function enviarDatosBloqueo() {
    ...
    if (datos.length > 0) {
        fetch("guardar_bloqueosUCIA.php", {
            method: "POST",
            headers: {
...
function cargarDatosBloqueo() {
    fetch("guardar_bloqueosUCIA.php") // o "obtener_bloqueosCG.php" si lo tienes separado
        .then(response => response.json())
        .then(data => {
            const filas = document.querySelectorAll("#miTabla tbody tr");
        ...
        fetch('importPacienteUCIA.php', {
            ...
            .then(text => {
                if (text.includes('Datos insertados correctamente.')) {
                    ...
                    //window.location.replace("http://localhost/dietas/formUCIA.php");
                    //window.location.replace("http://10.1.7.240:8080/dietas/formUCIA.php");
                    //window.location.replace("http://10.1.7.169/dietas/formUCIA.php");
