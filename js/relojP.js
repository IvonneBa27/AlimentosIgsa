        ...
        theadRows.forEach((tr, index) => {
            ...
            if (index === 0) {
                th.innerHTML = `<span>Solicitud día siguiente</span><br><button class="btn btn-success btn-sm mt-1" onclick="enviarDatosBloqueo()">Enviar datos</button>
                                 <br><button class="btn btn-primary btn-sm mt-1" onclick="window.open('pdfDesayunoP.php', '_blank')">Exportar</button>`;
            } else {
...
function cargarDatosBloqueo() {
    fetch("guardar_bloqueosP.php") // o "obtener_bloqueosCG.php" si lo tienes separado
        .then(response => response.json())
        .then(data => {
            const filas = document.querySelectorAll("#miTabla tbody tr");
        ...
        fetch('importPacienteP.php', {
            ...
            .then(text => {
                if (text.includes('Datos insertados correctamente.')) {
                    ...
                    //window.location.replace("http://localhost/dietas/formP.php");
                    //window.location.replace("http://10.1.7.240:8080/dietas/formP.php");
                    //window.location.replace("http://10.1.7.169/dietas/formP.php");

                } else if (text.includes('El ID ingresado para paciente ya existe')) {
                    alert(text);
                } else if (text.includes('Las siguientes camas no existen en el área de PEDIATRÍA')) {
                    alert(text);
                } else if (text.includes('La cama ingresada ya existe en el área de PEDIATRÍA')) {
                    alert(text);
                } else {
