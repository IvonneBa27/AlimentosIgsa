...
if ($query1->num_rows > 0) {
    ...
    $camaQuery = "SELECT * FROM pacientes WHERE cama = '$cama' AND area = 'GINECOLOGÍA' AND statusP = 'Activo'";
    $query2 = mysqli_query($con, $camaQuery);

    if ($query2->num_rows > 0) {

        $response['status'] = 'error';
        $response['message'] = 'La cama ingresada ya está ocupada en el área de GINECOLOGÍA, por favor elija otra cama';
    } else {
        $consulta = "INSERT INTO pacientes (id, nombre, fechaNacimiento, idPaciente, cama, edad, edadMeses, edadDias, diagnosticoMed, prescripcionNutri, area, vip, observaciones, controlTamizaje, creadoPor, statusP) 
            VALUES ('', '$nombrePaciente', '$fechaNac', '$idPaciente', '$cama', '$edadAnios', '$edadMeses', '$edadDias', '$diagMed', '$presNutri', 'GINECOLOGÍA', '$vip', '$observaciones', '$controlTami', '$nombreUsuario', '$statusP')";
