...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql = "UPDATE departamento SET estatus_id = 2 WHERE id = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: adminDepartamento.php?msg=updated");
                exit();
            } else {
                // Manejo de error si la consulta no se ejecutó correctamente
                echo 'Error al actualizar el estado del departamento: ' . $stmt->error;
            }
